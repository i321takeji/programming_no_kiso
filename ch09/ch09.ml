...
(* int list は - [] 空リスト，あるいは - first :: rest 最初の要素が first で残りのリストが rest
   という形 *)
...
(* length : 'a list -> int *)
let rec length lst = match lst with [] -> 0 | _ :: rest -> 1 + length rest
...
(* | first :: rest -> (match first with { namae = n; tensuu = t; seiseki = s
   } -> 0) *)
...
let p3 =
  {name= "hanako"; height= 1.6; weight= 50.0; birth= (3, 3); blood= "O"}
...
let rec otomeza' lst =
  ...
  | [] -> []
  | {name; birth= m, d} :: rest ->
      if seiza m d = "乙女座" then name :: otomeza' rest else otomeza' rest

(* テスト用データ *)
let p_virgo1 =
  {name= "miho"; height= 1.6; weight= 50.0; birth= (8, 23); blood= "A"}

let p_virgo2 =
  {name= "sota"; height= 1.7; weight= 60.0; birth= (9, 22); blood= "O"}

let p_not_virgo1 =
  {name= "ryo"; height= 1.75; weight= 68.0; birth= (8, 22); blood= "B"}

let p_not_virgo2 =
  {name= "kana"; height= 1.58; weight= 48.0; birth= (9, 23); blood= "AB"}
...
let test_otomeza5 =
  otomeza [p_not_virgo1; p_virgo1; p_virgo2; p_not_virgo2] = ["miho"; "sota"]
